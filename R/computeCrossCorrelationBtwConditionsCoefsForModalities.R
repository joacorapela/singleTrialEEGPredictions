...
computeCrossCorrelationBtwConditionsCoefsForModalities <- 
 function(sortvar, modalities, clustersIDs, condition1, condition2, 
    ...
    for(modality in modalities) {
        ...
        res <- 
         computeCrossCorrelationBtwConditionsCoefsForClusters(
          sortvar=sortvar,
          modality=modality,
       ...
       !is.na(c2MinSFPD) && !is.na(c2MaxSFPD)) {
        ...
        if(!is.null(c1AnalyzedData$lrtRes) &&
           ...
           c2AnalyzedData$lrtRes$pValue<modelSignificance) {
            res <- computeCrossCorrelationBtwConditionsCoefs(
...
computeCrossCorrelationBtwConditionsCoefs <- function(times1, condition1Coefs, 
    ...
    if(length(c1CommonTimeIndices)>=minNumberOfCommonIndices) {
        permRes <- permuteCrossCorrelation(x=condition1Coefs[c1CommonTimeIndices],
